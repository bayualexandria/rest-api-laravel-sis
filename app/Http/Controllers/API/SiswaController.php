...
class SiswaController extends Controller
{
    ...
    {
        $siswa = Siswa::where('status_siswa_id', 3)->get();
        if ($siswa->isEmpty()) {
            return response()->json(['message' => 'Data siswa tidak ditemukan', 'status' => 404], 404);
        }
    ...
    function getAllSiswaByKelas($kelasId)
    {
        $siswa = DB::table('history_siswa')
            ->join('siswa', 'history_siswa.siswa_id', '=', 'siswa.id')
            ->join('kelas', 'history_siswa.kelas_id', '=', 'kelas.id')
            ->select('history_siswa.id', 'siswa.nis', 'siswa.nama', 'kelas.nama_kelas', 'kelas.jurusan')
            ->where('kelas_id', $kelasId)
            ->where('history_siswa.deleted_at', null)
            ->get();
    ...
    function insertSiswaByKelas(Request $request)
    {
        ...
        $this->historySiswa->create($request->all());
        $this->siswa->where('id', $request->siswa_id)->update(['status_siswa_id' => 2]);
        return response()->json(['message' => "Data berhasil ditambahkan!", 'status' => 200], 200);
    }

    function updateSiswaByKelas(Request $request, $id)
    {
        $historySiswa = DB::table('history_siswa')->where('id', $id)->first();

        $historySiswa->update([
            'siswa_id' => $request->siswa ? $request->siswa : $historySiswa->siswa_id,
            'kelas_id' => $request->kelas ? $request->kelas : $historySiswa->kelas_id,
            'mapel_id' => $request->mapel ? $request->mapel : $historySiswa->mapel_id
        ]);
        return response()->json(['message' => "Data berhasil diubah!", 'status' => 200], 200);
    }

    function deleteSiswaByKelas($kelas)
    {
        $historySiswa = $this->historySiswa->where('id', $kelas)->first();
        if (!$historySiswa) {
            return response()->json(['message' => 'Data yang anda hapus tidak ada!', 'status' => 403], 403);
        }
        $this->siswa->where('id',  $historySiswa->siswa_id)->update(['status_siswa_id' => 3]);
        $historySiswa->delete();

        return response()->json(['data' => $historySiswa, 'message' => "Data berhasil dihapus!", 'status' => 200], 200);
    }
}
