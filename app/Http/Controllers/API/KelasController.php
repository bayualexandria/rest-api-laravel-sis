...
class KelasController extends Controller
{
    ...
    {
        $historyKelas = HistorySiswa::where('kelas_id', $kelas);
        $status = HistorySiswa::where('kelas_id', $kelas)->get();
        $siswas = Siswa::whereIn('id', $status->pluck('siswa_id'))->get();
        
        foreach ($siswas as $siswa) {
            $siswa->status_siswa_id = 3;
            $siswa->save();
        }
        $historyKelas->delete();
        $data = HistoryKelas::where('id', $kelas)->first();
        $data->delete();
